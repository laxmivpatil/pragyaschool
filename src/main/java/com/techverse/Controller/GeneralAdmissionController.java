...
public class GeneralAdmissionController {
    ...
    public ResponseEntity<?> createAdmission( 
             ...
             @RequestPart("admissionClass") String admissionClass,
    	...
    	if(type.equals("general")) {
    		 GeneralAdmission createdAdmission = new GeneralAdmission(firstName,lastName, gender,dateOfBirth,
    		 ...
    		 String body = "<html><body>" +
    			        ...
    			        "<p>Details of the applicant are as follows:</p>" +
    			        "<ul>" +
    			        "<li>Full Name: " + firstName + " " + lastName + "</li>" +
    			        "<li>Gender: " + gender + "</li>" +
    			        "<li>Date of Birth: " + dateOfBirth + "</li>" +
    			        "<li>Class Applying For: " + admissionClass + "</li>" +
    		...
    		String birthC="",lastR="",parentA="",studentA="", sssmi="", bankD="", castC="",transferC="", profileP="";
    		//name
    		String birthCN="",lastRN="",parentAN="",studentAN="", sssmiN="", bankDN="", castCN="",transferCN="", profilePN="";
    		//extension
    		...
    		if (birthCertificate != null && !birthCertificate.isEmpty()) {
    	       ...
    	       birthCN = "Birth_Certificate" + " " + Instant.now().toEpochMilli();
    	        ...
    	        birthC = storageService.uploadFileOnAzure(birthCertificate, birthCN+"."+birthT );
    	    ...
    	    if (parentAadhar != null && !parentAadhar.isEmpty()) {
    	    	parentAT= getExtension(parentAadhar);
    	        parentAN = "Parent_Aadhar" + " " + Instant.now().toEpochMilli();
    	        parentA = storageService.uploadFileOnAzure(parentAadhar,parentAN+"."+parentAT);
    	    }

    	    if (studentAadhar != null && !studentAadhar.isEmpty()) {
    	    	studentAT = getExtension(studentAadhar);
    	    	studentAN = "Student_Aadhar" + " " + Instant.now().toEpochMilli();
    	        studentA = storageService.uploadFileOnAzure(studentAadhar, studentAN+"."+studentAT );
    	    }

    	    if (bankDoc != null && !bankDoc.isEmpty()) {
    	    	bankDT = getExtension(bankDoc);
    	    	bankDN = "Bank_Doc" + " " + Instant.now().toEpochMilli() ;
    	        bankD = storageService.uploadFileOnAzure(bankDoc, bankDN+"."+bankDT);
    	    }

    	    if (cast != null && !cast.isEmpty()) {
    	       castCT = getExtension(cast);
    	       castCN = "Cast" + " " + Instant.now().toEpochMilli();
    	        castC = storageService.uploadFileOnAzure(cast, castCN+"."+castCT);
    	    }

    	    if (transferCertificate != null && !transferCertificate.isEmpty()) {
    	    	transferCT= getExtension(transferCertificate);
    	    	transferCN = "Transfer_Certificate" + " " + Instant.now().toEpochMilli();
    	        transferC = storageService.uploadFileOnAzure(transferCertificate, transferCN+"."+transferCT);
    	    }

    	    if (profile != null && !profile.isEmpty()) {
    	    	profilePT = getExtension(profile);
    	        profilePN = "Profile" + " " + Instant.now().toEpochMilli() ;
    	        profileP = storageService.uploadFileOnAzure(profile, profilePN+"."+profilePT);
    	    }

    	    if (sssmid != null && !sssmid.isEmpty()) {
    	       sssmiT = getExtension(sssmid);
    	        sssmiN = "SSSMID" + " " + Instant.now().toEpochMilli() ;
    	        sssmi = storageService.uploadFileOnAzure(sssmid, sssmiN+"."+sssmiT);
    	    }
    		
    		  GeneralAdmission createdAdmission = new GeneralAdmission(firstName,lastName, gender,dateOfBirth,
    				 admissionClass,fatherName, motherName, mobileNo, email, type,
    				  pen,   birthC,  lastR,parentA, studentA,
    				 sssmi,  bankD,  castC, transferC,  profileP);
    		  String subject = "New Admission Request";
    		 ...
    		 String body = "<html><body>" +
 			       ...
 			       "<li>PEN: " + pen + "</li>" +
 			        ...
 			        "<li>Previous Year's Result</li>" +
 			        "<li>Parent's Aadhar Card</li>" +
